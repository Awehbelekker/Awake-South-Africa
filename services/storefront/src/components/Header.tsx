...
export default function Header() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled ? "bg-awake-black/95 backdrop-blur-md shadow-lg border-b border-white/5" : "bg-transparent"
    }`}>
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navigation.map((item) => (
              <div key={item.name} className="relative"
                ...
                <Link href={item.href} className={`flex items-center gap-1 text-sm font-medium transition-colors ${
                  activeDropdown === item.name ? "text-accent-primary" : "text-gray-300 hover:text-white"
                }`}>
                  {item.name}
                  {item.submenu && <ChevronDown className={`w-4 h-4 transition-transform ${activeDropdown === item.name ? "rotate-180" : ""}`} />}
                </Link>
          ...
          <div className="flex items-center gap-4">
            <Link href="/account/wishlist" className="relative p-2 text-gray-400 hover:text-white transition-colors">
            ...
            {isAuthenticated ? (
              <Link href="/account" className="hidden md:flex items-center gap-2 px-4 py-2 text-sm text-gray-300 hover:text-white">
                <User className="w-5 h-5" />
                <span>{customer?.first_name || "Account"}</span>
              </Link>
            ) : (
              <Link href="/login" className="hidden md:flex px-4 py-2 text-sm font-medium text-awake-black bg-accent-primary rounded-lg hover:bg-accent-primary/90">
                Sign In
              </Link>
            )}
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="lg:hidden p-2 text-gray-400 hover:text-white">
          ...
          <div className="lg:hidden fixed inset-0 top-20 bg-awake-black/98 backdrop-blur-lg z-50">
            <div className="px-4 py-8 space-y-4">
              ...
              <div className="pt-8 border-t border-white/10">
                {isAuthenticated ? (
                  <Link href="/account" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 py-3 text-white">
